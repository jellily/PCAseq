                      ...
                      maf, missing.rate)
{
  ...
  for(i in 1:max)
  {
    ...
    # Filter the data
    snp.dat <- filter.snps(snp.dat, autosomal.only, remove.monosnp, missing.rate, maf)
    ...
    geno.cent <- sweep(snp.dat, bycolumns, STATS = allele.freq)   
  ...
  sigma.pcaseq <- ncopies*allele.ps*(1-allele.ps) # standard deviation
