                     ...
                     maf, missingRate, transpose)
{
  ...
  for(i in 1:max)
  {
    ...
    snpDat <- filterSnps(snpDat, autosomeOnly, removeMonosnp, missingRate, 
                          maf, read.gdsn(index.gdsn(geno.dat, 
                                                    "snp.chromosome")))
                      ...
                      maf, missingRate, transpose)
{
  ...
  for(i in 1:max)
  {
    ...
    # Filter the data
    snp.dat <- filterSnps(snpDat, autosomeOnly, removeMonosnp, missingRate, 
...
getInxed <- function(i, nBlock, nSnps)
{
